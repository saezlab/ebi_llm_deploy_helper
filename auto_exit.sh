...
if command -v nvidia-smi &> /dev/null; then
    ...
    echo "  GPU Configuration: ${gpu_info}"
else
...
echo ""
echo "      The above command can also be used to reattach after disconnection (e.g., laptop sleep)."
echo ""
echo ""
echo "  [2] Launch OpenAI OSS-120B using vLLM:"
echo ""
echo "      saez_home=/hps/nobackup/saezrodriguez && HF_HOME=\$saez_home/shared_hf_home && singularity exec --nv \$saez_home/singularity_images/vllm-openai_latest.sif \\"
echo "        vllm serve openai/gpt-oss-120b --tensor-parallel-size $gpu_count --port $available_port \\"
echo "          --enable-auto-tool-choice --tool-call-parser openai --reasoning-parser openai_gptoss"
echo ""
echo "      For more information, refer to the README.md at https://github.com/saezlab/ebi_llm_deploy_helper."
echo ""
echo ""
echo "  [3] Once your server has started, the base URL of the OpenAI compatible API is:"
echo ""
echo "      http://localhost:$available_port/v1/"
echo ""
echo ""
echo "  [4] You can ignore this terminal or close it — it will exit automatically once"
echo "      the server session ends. If you want to release the compute node manually,"
